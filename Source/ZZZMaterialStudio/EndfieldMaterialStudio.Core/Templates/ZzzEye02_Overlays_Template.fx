// ZZZ Eye 02: Jane's authored eye overlay geometry.
// PMX 9 = inner light, 10 = outer highlight, 11 = eye shadow.
// Each layer keeps its own authored opacity and remains independent from Eye 01.
    ...
    float baseMaterialAlpha)
{
    ...
    float3 baseSrgb = useTexture
        ...
        : materialColor.rgb;
    // Preserve the authored opacity while still allowing PMX material
    // morphs to hide a layer. Eye shadow already stores 0.5 in PMX, whereas
    // inner light stores 1.0 and receives its 0.5 from the source material.
...
ZZZ_EYE_OVERLAY_TECHNIQUE(
    ...
    false, ONE)
ZZZ_EYE_OVERLAY_TECHNIQUE(
    ZzzEyeInnerObjectSs, "object_ss", "7", true, ZzzEyeInnerVS, ZzzEyeInnerPS,
    false, ONE)
