...
texture ZZZshadow_VMap : OFFSCREENRENDERTARGET <
    ...
    string DefaultEffect = 
        "self = hide;"
        "* = ZZZshadow_ViewportMap.fxsub;"
        ;
>;
#endif

// オフスクリーンシャドウマップバッファ
shared texture ZZZshadow_SMap : OFFSCREENRENDERTARGET <
...
// シャドウ描画編集用のレンダーターゲット
texture2D ZZZshadow_ViewportMap1 : RENDERCOLORTARGET <
    float2 ViewPortRatio = {1.0, 1.0};
    int MipLevels = 1;
    string Format = TEX_FORMAT_WMAP;
>;
sampler2D WorkMapSamp1 = sampler_state {
    texture = <ZZZshadow_ViewportMap1>;
    MinFilter = LINEAR;
    MagFilter = LINEAR;
    MipFilter = NONE;
    AddressU  = CLAMP;
...
texture2D ZZZshadow_DepthStencilBuffer : RENDERDEPTHSTENCILTARGET <
    float2 ViewPortRatio = {1.0, 1.0};
...
technique MainTech < string MMDPass = "object";
    string Script = 
        #if SoftShadowQuality > 0
        "RenderColorTarget0=ZZZshadow_ViewportMap1;"
        #else
        "RenderColorTarget0=ZZZshadow_ViewportMap2;"
        #endif
            "RenderDepthStencilTarget=ZZZshadow_DepthStencilBuffer;"
